...
import com.idkstudios.game.utils.IntegerPool;
...
public class ChunkData {
	...
	public void setDefaultBlock(int index, byte type, byte faceMask,
			byte metaData) {
		if (isSpecial(index)) {
	...
	public void setMetaData(int index, byte metadata) {
		if (isSpecial(index)) {
			System.out.println("CHUNKDATA: Setting metadata for special block!!!");
		}
	...
	public void setPosition(int index, int position) {
		if (!isSpecial(index)) {
			System.out.println("CHUNKDATA: Updating position of a non-special block");
		}
		index *= BLOCK_DATA_SIZE;
		++index;
	...
	public int getPosition(int index) {
		index *= BLOCK_DATA_SIZE;
		++index;
		return ((_data[index] & (~SPECIAL_BIT)) << 8)
				| (_data[index + 1] & 0xFF);
	}
	...
	public void clearBlock(int index) {
		...
		if (special) {
			System.out.println("CHUNKDATA: Removing special block!");
	...
	public static int positionToIndex(int x, int y, int z) {
		return x * Chunk.CHUNK_SIZE_VERTICAL * Chunk.CHUNK_SIZE_HORIZONTAL + y
				* Chunk.CHUNK_SIZE_HORIZONTAL + z;
	}

	public static Vec3i indexToPosition(int index, Vec3i output) {
		output.setX(index
				/ (Chunk.CHUNK_SIZE_HORIZONTAL * Chunk.CHUNK_SIZE_VERTICAL));
		output.setY((index / Chunk.CHUNK_SIZE_HORIZONTAL)
