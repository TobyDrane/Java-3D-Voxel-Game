...
public class FloatingIslandGenerator extends Generator {
	...
	public FloatingIslandGenerator(WorldProvider provider) {
		_dirt = blockManager.getBlockType("dirt");
		_grass = blockManager.getBlockType("grass");
		_provider = provider;
	}
	...
	public void generateNiceFloatingIsland(Chunk chunk, int ix, int iy, int iz) {
		...
		_random = new SmartRandom(new Random(worldSeed + ix * 1024L + iy
				* 512L + iz));
		...
		for (int x = 0; x < dimX; ++x) {
			for (int y = 0; y < dimY; ++y) {
				for (int z = 0; z < dimZ; ++z) {
					float noise = 100.0f * _pNoise.noise((float) x / dimX,
							(float) y / dimY, (float) z / dimZ);
					if (noise > 0.25f) {
						chunk.setDefaultBlockAbsolute(ix + x, iy + y, iz + z,
								_dirt, (byte) 0, true, true, false);
					}
				}
			}
		}
	...
	public void generateFloatingIsland(Chunk chunk, int ix, int iy, int iz) {
		...
		int points = MathHelper.ceil(_random.randomFloat(size * size / 2.0f,
				size * size / 1.5f + 4));
		...
		int radius = MathHelper.ceil((float) Math.sqrt(polygon.getBounds2D()
				.getWidth() * polygon.getBounds2D().getHeight()));
		...
		for (int x = -radius; x <= radius; ++x) {
			for (int z = -radius; z <= radius; ++z) {
				if (polygon.contains(x, z)) {
					...
					/* Generate land here */
					int height = _provider
							.getHeightAt(ix + x + 20, iz + z - 20)
							- referenceHeight;
					int depth = Math.abs(MathHelper.round((MathHelper.cos(x * z
							/ 4.0f)
							* MathHelper.sin(x * z / 8.0f) * (size
							- MathHelper.sqrt(x * x + z * z) - 2))));
					for (int i = -depth - 2; i < height; ++i) {
						chunk.setDefaultBlockAbsolute(ix + x, iy + i, iz + z,
								_dirt, (byte) 0, true, true, false);
					}
					chunk.setDefaultBlockAbsolute(ix + x, iy + height, iz + z,
							_grass, (byte) 0, true, true, false);
					...
					if (!treeBuilt && x * x + z * z < size * size - 5) {
						if (_random.randomInt(80) == 0) {
							...
							TreeGenerator gen = new TreeGenerator(
									_random.randomLong());
							gen.generateNiceBroadLeavedTree(chunk, ix + x, iy
									+ height, iz + z);
						}
					}
				}
