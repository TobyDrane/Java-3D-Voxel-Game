...
import com.idkstudios.game.datastructures.Fast3DByteArray;
...
public class LightBuffer {
	...
	public LightBuffer() {
		_buffer = new Fast3DByteArray(Chunk.CHUNK_SIZE_HORIZONTAL + 2,
				Chunk.CHUNK_SIZE_VERTICAL, Chunk.CHUNK_SIZE_HORIZONTAL + 2);
	}
	...
	public void buffer(Chunk chunk) {
		...
		int minY = MathHelper.clamp(
				MathHelper.floor(chunk.getVisibleContentAABB().minY() - 2), 0,
				Chunk.CHUNK_SIZE_VERTICAL);
		int maxY = MathHelper.clamp(
				MathHelper.ceil(chunk.getVisibleContentAABB().maxY() + 2), 0,
				Chunk.CHUNK_SIZE_VERTICAL);

		int i = 0;
		for (int x = -1; x <= Chunk.CHUNK_SIZE_HORIZONTAL; ++x) {
			for (int z = -1; z <= Chunk.CHUNK_SIZE_HORIZONTAL; ++z) {
				if (x != -1 && x != Chunk.CHUNK_SIZE_HORIZONTAL && z != -1
						&& z != Chunk.CHUNK_SIZE_HORIZONTAL) {
					for (int y = minY; y <= maxY; ++y) {
						...
						_buffer.set(x + 1, y, z + 1,
								(byte) Math.max(blockLight * 2, sunlight));
					}
				} else {
					Chunk c = chunk.getChunkContaining(_x + x, maxY, _z + z,
							false, false, false);
					if (c == null)
						continue;
					...
					int xx = MathHelper
							.simplify(x, Chunk.CHUNK_SIZE_HORIZONTAL);
					int zz = MathHelper
							.simplify(z, Chunk.CHUNK_SIZE_HORIZONTAL);
