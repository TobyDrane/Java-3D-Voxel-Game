...
public class ItemXMLLoader {
		...
		for (int i = 0; i < blocksList.getLength(); ++i) {
			...
			if (node.getNodeType() == Node.ELEMENT_NODE) {
				try {
					...
					ItemManager.getInstance().putInventoryItem(
							item.getInventoryTypeID(), item);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	private static InventoryItem parseItem(Element element) throws Exception {
		...
		if (!className.equals("minecraft.world.items.GenericItem")) {
			...
			Class<? extends InventoryItem> cl = (Class<? extends InventoryItem>) Class
					.forName(className);
		...
		return new GenericItem(name, animationSpeed, stackable,
				TextureStorage.getTexture("items"), tex);
