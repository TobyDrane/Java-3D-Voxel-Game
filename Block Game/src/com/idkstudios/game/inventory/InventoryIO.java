...
import com.idkstudios.game.inventory.Inventory.InventoryItemStack;
import com.idkstudios.game.inventory.Inventory.InventoryPlace;
import com.idkstudios.game.items.ItemManager;

public class InventoryIO {

	public static void writeInventory(Inventory inv, DataOutputStream dos,
			int offset, int length) throws IOException {
		if (offset + length > inv.size()) {
			throw new IndexOutOfBoundsException(
					"Inventory index out of bounds!");
		}
	...
	public static void readInventory(DataInputStream dis, Inventory inv,
		...
		for (int i = offset; i < offset + length; ++i) {
			...
			if (type == 0) {
				...
				if (type == 2) {
					...
					inv.setContentAt(new InventoryPlace(i,
							inv.new InventoryItemStack(stackItemType,
									stackItemCount)), i);
				} else if (type == 1) {
					short itemType = dis.readShort();
					float health = dis.readFloat();
					// TODO use the health
					inv.setContentAt(new InventoryPlace(i, ItemManager
							.getInstance().getInventoryItem(itemType)), i);
				} else {
