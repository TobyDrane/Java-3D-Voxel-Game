...
public class InventoryImageCreator {
	...
	public InventoryImageCreator(int size) {
		...
		NOT_AVAILABLE = new BufferedImage(size, size,
	...
	public BufferedImage createInventoryImage(BlockType type) {
		...
		if (type.getCustomInventoryImage() != null
				&& type.getCustomInventoryImageTexture() != null) {
			Texture tex = TextureStorage.getTexture(type
					.getCustomInventoryImageTexture());
			BufferedImage img = textureToBufferedImage(tex);
			img = getTile16(img, type.getCustomInventoryImage().x(), type
					.getCustomInventoryImage().y());
			return img;
		} else {
			if (type.isCrossed()) {
	...
	public BufferedImage createInventoryImage(CrossedBlockBrush cbb) {
		BufferedImage bi = new BufferedImage(_size, _size,
				BufferedImage.TYPE_INT_ARGB);

		Vec2f texpos = cbb.getTexturePosition();
		BufferedImage tile = getTile16(_terrainImage,
				MathHelper.round(texpos.x() / 16f * _terrainImage.getWidth()),
				MathHelper.round(texpos.y() / 16f * _terrainImage.getHeight()));
	...
	public BufferedImage createInventoryImage(DefaultBlockBrush dbb) {
		...
		// Left
		{
			...
			BufferedImage leftSide = getTile16(_terrainImage,
					(int) leftPos.x(), (int) leftPos.y());
			leftSide = applyColorFilter(leftSide, leftColor.x(), leftColor.y(),
					leftColor.z());
		...
		// Front
		{
			...
			transform.scale(0.5, 0.5);
			transform.shear(0.0, -0.36);
			BufferedImage frontSide = getTile16(_terrainImage,
					(int) frontPos.x(), (int) frontPos.y());
			frontSide = applyColorFilter(frontSide, frontColor.x(),
					frontColor.y(), frontColor.z());
		...
		// Top
		{
			...
			BufferedImage topSide = getTile16(_terrainImage, (int) topPos.x(),
					(int) topPos.y());
			topSide = applyColorFilter(topSide, topColor.x(), topColor.y(),
					topColor.z());
	...
	private static BufferedImage getTile16(BufferedImage _terrainImage, int x,
			int y) {
		return _terrainImage.getSubimage(x * 16, y * 16, 16, 16);
	}

	private static BufferedImage applyColorFilter(BufferedImage img, float r,
			float g, float b) {
