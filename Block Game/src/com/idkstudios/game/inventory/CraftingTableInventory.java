...
public class CraftingTableInventory extends Inventory {
	...
	private static final int inventoryDrawList;
	private static final Vec2f texPosUpLeft;
	private static final Vec2f texPosDownRight;
	private static final Texture tex;
	private static final int hw = 176;
	private static final int hh = 167;

	/** INVENTORY VARIABLES */
	@SuppressWarnings("unused")
	private boolean validRecipePresent = false;
	...
	static {
		...
		tex = TextureStorage.getTexture("gui.crafting");
		...
		inventoryDrawList = GL11.glGenLists(1);
		GL11.glNewList(inventoryDrawList, GL11.GL_COMPILE);

		GL11.glColor3f(1, 1, 1);
		GL11.glBegin(GL11.GL_QUADS);
		GL11.glTexCoord2f(texPosUpLeft.x(), texPosUpLeft.y());
		GL11.glVertex2f(-hw, hh);
		GL11.glTexCoord2f(texPosDownRight.x(), texPosUpLeft.y());
		GL11.glVertex2f(hw, hh);
		GL11.glTexCoord2f(texPosDownRight.x(), texPosDownRight.y());
		GL11.glVertex2f(hw, -hh);
		GL11.glTexCoord2f(texPosUpLeft.x(), texPosDownRight.y());
		GL11.glVertex2f(-hw, -hh);
		GL11.glEnd();
	...
	public static void UNLOAD_STATIC_CONTENT() {
		GL11.glDeleteLists(inventoryDrawList, 1);
	}

	public CraftingTableInventory() {
		super(46);
		raster = new CraftingTableInventoryRaster();
	}

	/**
	 * Checks if there is a valid recipe on the table by comparing with every
	 * recipe from the recipe manager.
	 */
		...
		if (r != null) {
			...
			if (item.isStackable()) {
				setContentAt(new InventoryPlace(0, new InventoryItemStack(type,
						r.getResultAmount())),
						CraftingTableInventoryRaster.CRAFTING_OFFSET + 9);
			} else {
	...
	/**
	 * @return A 3x3 int matrix representing the table. The int values are the
	 *         Item ID's.
	 */
	...
	public void renderInventory() {
		...
		GL11.glCallList(inventoryDrawList);
		GL11.glPopMatrix();
	...
	protected void inventoryEvent(InventoryEvent evt) {
		...
		if (evt.getAction() == InventoryEvent.DROP
				&& evt.getIndex() >= CraftingTableInventoryRaster.CRAFTING_OFFSET) {
			checkForRecipe();
			...
			for (int y = 0; y < 3; ++y) {
				for (int x = 0; x < 3; ++x) {
					int index = CraftingTableInventoryRaster.CRAFTING_OFFSET
							+ (y * 3) + x;
		...
		Game.getInstance().getWorld().getActivePlayer()
	...
	public static final class CraftingTableInventoryRaster implements
			InventoryRaster {
