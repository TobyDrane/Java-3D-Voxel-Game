...
public class RayBlockIntersection {
	...
	public static class Intersection implements Comparable<Intersection> {
		...
		public Intersection(int blockX, int blockY, int blockZ, Vec3i normal,
				float d, float t, Vec3f rayOrigin, Vec3f rayDirection,
		...
		public String toString() {
			return String
					.format("x: %d y: %d z: %d", blockX, blockY, blockZ);
		}
	...
	/**
	 * Calculates the intersection of a given ray originating from a specified
	 * point with a block. Returns a list of intersections ordered by the
	 * distance to the player.
	 * 
	 * @param w
	 *            The world provider
	 * @param x
	 *            The block's position on the x-axis
	 * @param y
	 *            The block's position on the y-axis
	 * @param z
	 *            The block's position on the z-axis
	 * @param rayOrigin
	 *            The origin of the ray
	 * @param rayDirection
	 *            The direction of the ray
	 */
	public static ArrayList<Intersection> executeIntersection(int x, int y,
			int z, AABB blockAABB, Vec3f rayOrigin, Vec3f rayDirection) {
		...
		// Front
		Intersection is = executeBlockFaceIntersection(x, y, z, vertices[0],
				vertices[1], vertices[3], rayOrigin, rayDirection);
		if (is != null) {
			result.add(is);
		}

		// Back
		is = executeBlockFaceIntersection(x, y, z, vertices[4], vertices[7],
				vertices[5], rayOrigin, rayDirection);
		if (is != null) {
			result.add(is);
		}

		// Left
		is = executeBlockFaceIntersection(x, y, z, vertices[4], vertices[0],
				vertices[7], rayOrigin, rayDirection);
		if (is != null) {
			result.add(is);
		}

		// Right
		is = executeBlockFaceIntersection(x, y, z, vertices[5], vertices[6],
		...
		// Top
		is = executeBlockFaceIntersection(x, y, z, vertices[7], vertices[3],
				vertices[6], rayOrigin, rayDirection);
		if (is != null) {
			result.add(is);
		}

		// Bottom
		is = executeBlockFaceIntersection(x, y, z, vertices[4], vertices[5],
				vertices[0], rayOrigin, rayDirection);
		if (is != null) {
			result.add(is);
		}
	...
	/**
	 * Calculates an intersection with the face of a block defined by three
	 * points.
	 * 
	 * @param blockPos
	 *            The position of the block to intersect with
	 * @param v0
	 *            Point 1
	 * @param v1
	 *            Point 2
	 ...
	 */
	private static Intersection executeBlockFaceIntersection(int blockX,
			int blockY, int blockZ, Vec3f v0, Vec3f v1, Vec3f v2, Vec3f origin,
			Vec3f ray) {
		 ...
		 */
		float t = -(norm.x() * origin.x() + norm.y() * origin.y() + norm.z()
				* origin.z() + d)
				/ Vec3f.dot(ray, norm);
		 ...
		 */
		Vec3f intersectPoint = new Vec3f((ray.x() * t), (ray.y() * t),
				(ray.z() * t));
		...
		if (intersectPoint.x() >= v0.x()
			...
			return new Intersection(blockX, blockY, blockZ, new Vec3i(
					(int) norm.x(), (int) norm.y(), (int) norm.z()), d, t,
					origin, ray, intersectPoint);
		}
