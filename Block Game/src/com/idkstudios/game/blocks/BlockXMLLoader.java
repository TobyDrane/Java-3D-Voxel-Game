...
public class BlockXMLLoader {
	...
	private static BlockType parseBlockType(Element element) {
		...
		/* BlockBrush */
		Element defaultBrushElement = (Element) element.getElementsByTagName(
				"brush").item(0);
		if (defaultBrushElement != null) {
			DefaultBlockBrush bb = new DefaultBlockBrush();
			if (defaultBrushElement.hasAttribute("alphaBlending")) {
				bb.setAlphaBlending(Boolean.parseBoolean(defaultBrushElement
						.getAttribute("alphaBlending")));
			...
			for (int i = 0; i < brushSidesList.getLength(); ++i) {
				if (brushSidesList.item(i).getNodeType() == Node.ELEMENT_NODE) {
					...
					if (sideElement.hasAttribute("inset")) {
						inset = Float.parseFloat(sideElement
								.getAttribute("inset"));
					}
		...
		Element crossedBrushElement = (Element) element.getElementsByTagName(
				"crossedbrush").item(0);
		...
		for (int i = 0; i < settingsList.getLength(); ++i) {
			if (settingsList.item(i).getNodeType() == Node.ELEMENT_NODE) {
				...
				if (settingName.equals("mineResult")) {
					if (valueStr.equals("null")) {
						...
						if (valueStr.matches("\\d+")) {
							bm.setBlockTypeSetting(blockType, settingName,
									Integer.parseInt(valueStr));
						} else {
							bm.setBlockTypeSetting(
									blockType,
									settingName,
									ItemManager.getInstance().getItemID(
											valueStr));
						}
					}
					continue;
				...
				if (settingName.equals("customInventoryImage")) {
					bm.setBlockTypeSetting(blockType, settingName,
							parseVec2i(valueStr));
					bm.setBlockTypeSetting(blockType, settingName + "Texture",
							settingsElement.getAttribute("texture"));
					continue;
				}
