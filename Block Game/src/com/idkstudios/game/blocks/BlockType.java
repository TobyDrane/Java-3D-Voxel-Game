...
public final class BlockType extends InventoryItem {
	...
	public void renderInventoryItem() {
		...
		if (customInventoryImage != null) {
			Vec2f uv0 = new Vec2f(customInventoryImage.x()
					/ (inventoryTexture.getImageWidth() / 16.0f),
					customInventoryImage.y()
							/ (inventoryTexture.getImageHeight() / 16.0f));

			glBegin(GL_QUADS);
			glTexCoord2f(uv0.x(), uv0.y());
			glVertex2f(-hw, hh);
			glTexCoord2f(uv0.x() + (16.0f / inventoryTexture.getImageWidth()),
					uv0.y());
			glVertex2f(hw, hh);
			glTexCoord2f(uv0.x() + (16.0f / inventoryTexture.getImageWidth()),
					uv0.y() + (16.0f / inventoryTexture.getImageHeight()));
			glVertex2f(hw, -hh);
			glTexCoord2f(uv0.x(),
					uv0.y() + (16.0f / inventoryTexture.getImageHeight()));
			glVertex2f(-hw, -hh);
