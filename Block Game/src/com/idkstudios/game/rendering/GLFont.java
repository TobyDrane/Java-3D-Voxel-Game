...
public class GLFont {
	...
	public int makeFontTexture(Font f, float[] fgColor, float[] bgColor) {
		...
		try {
			...
			this.texture = TextureStorage.loadTexture(
					"font-" + f.getFontName(), image);
	 ...
	 */
	public static int makeCharTexture(Font f, String onechar, float[] fgColor,
			float[] bgColor) {
		int texture = 0;
		try {
			...
			Texture tex = TextureStorage.loadTexture("Font-" + f.getFontName()
					+ "-" + onechar, image);
	 ...
	 * @return
	 */
	public static BufferedImage createCharImage(String text, Font font,
			float[] fgColor, float[] bgColor) {
		Color bg = bgColor == null ? new Color(0, 0, 0, 0)
				: (bgColor.length == 3 ? new Color(bgColor[0], bgColor[1],
						bgColor[2], 1) : new Color(bgColor[0], bgColor[1],
						bgColor[2], bgColor[3]));
		Color fg = fgColor == null ? new Color(1, 1, 1, 1)
				: (fgColor.length == 3 ? new Color(fgColor[0], fgColor[1],
						fgColor[2], 1) : new Color(fgColor[0], fgColor[1],
						fgColor[2], fgColor[3]));
		...
		if (imgSize > 2048) {
			throw new RuntimeException(
					"GLFont.createCharImage(): texture size will be too big ("
							+ imgSize + ") Make the font size smaller.");
		}
		...
		g.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING,
				isAntiAliased ? RenderingHints.VALUE_TEXT_ANTIALIAS_ON
	 ...
	 * @return
	 */
	public BufferedImage createFontImage(Font font, float[] fgColor,
			float[] bgColor) {
		Color bg = bgColor == null ? new Color(0, 0, 0, 0)
				: (bgColor.length == 3 ? new Color(bgColor[0], bgColor[1],
						bgColor[2], 1) : new Color(bgColor[0], bgColor[1],
		...
		System.out
				.println("GLFont.getFontImageSize(): build font with fontsize="
						+ fontSize + " gridsize=" + (fontSize * 10)
						+ " texturesize=" + textureSize);
		if (textureSize > 2048) {
			throw new RuntimeException(
					"GLFont.createFontImage(): texture size will be too big ("
							+ textureSize + ") Make the font size smaller.");
		}
		...
		for (int r = 0; r < 10; r++) {
			for (int c = 0; c < 10; c++) {
				...
				g.drawString(String.valueOf(ch), (c * fontSize), (r * fontSize)
						+ ascent);
