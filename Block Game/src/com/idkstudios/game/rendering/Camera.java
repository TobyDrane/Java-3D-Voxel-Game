...
public class Camera {
	...
	public void lookThrough(float viewingDistance) {
		...
		// Perspective.
		float widthHeightRatio = (float) Display.getWidth()
		...
		GLU.gluLookAt(x, y + Math.abs(bobbing) * 3.5f, z,
				x + lookDirection.x(),
				y + lookDirection.y() + Math.abs(bobbing) * 3.5f, z
						+ lookDirection.z(), up.x() + right.x(),
				up.y() + right.y(), up.z() + right.z());
	...
	public void setRotation(float rotX, float rotY, float bobbing) {
		...
		this.lookDirection.set(MathHelper.cos(rotY), MathHelper.tan(rotX),
				-MathHelper.sin(rotY));
