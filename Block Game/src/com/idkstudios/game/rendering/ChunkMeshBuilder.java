...
public class ChunkMeshBuilder {
	...
	public static void generateChunkMesh(Chunk chunk, MeshType meshType) {
		if (DEBUG)
			System.out.println("Building " + meshType.name() + " Mesh for "
					+ chunk.toString() + "...");
		...
		ByteBuffer byteBuffer = GL15.glMapBuffer(GL15.GL_ARRAY_BUFFER,
				GL15.GL_WRITE_ONLY, size, null);
		if (byteBuffer == null) {
			System.out
					.println("\tCouldn't create a native VBO!: GL Error Code = "
							+ GL11.glGetError());
		...
		for (int i = 0; i < blockList.size(); ++i) {
			...
			if ((meshType == MeshType.OPAQUE && !type.isTranslucent() && type
					.hasNormalAABB())
					|| (meshType == MeshType.TRANSLUCENT && (type
							.isTranslucent() || !type.hasNormalAABB()))) {
				...
				if (special) {
					...
					if (type.isCrossed()) {
						type.getCrossedBlockBrush().storeInVBO(vertexBuffer,
						...
						type.getBrush().storeInVBO(vertexBuffer,
								vec.x() + 0.5f, vec.y() + 0.5f, vec.z() + 0.5f,
								lightBuffer);
					}
				}
		...
		if (USED_SIZE != STRIDE * FLOAT_SIZE * mesh.getVertexCount(meshType)) {
			...
			System.out.println("\t[WARNING!]: Vertex count = " + USED_SIZE
					/ STRIDE / FLOAT_SIZE);
	...
	public static void putColorWithLight4(FloatBuffer vertexBuffer, Vec3f vec,
			byte light, byte light1, byte light2, byte light3) {
		float value;
	...
	public static void putColorWithLight3(FloatBuffer vertexBuffer, Vec3f vec,
			byte light, byte light1, byte light2) {
		float value;
	...
	public static void putColorWithLight(FloatBuffer vertexBuffer, Vec3f vec,
			byte light) {
		float value;
		value = light / 30.001f;
		vertexBuffer.put(vec.x() * value);
	...
	public static void put3f(FloatBuffer vertexBuffer, float f0, float f1,
			float f2) {
		vertexBuffer.put(f0);
		vertexBuffer.put(f1);
		vertexBuffer.put(f2);
